...
/**
 * A client that runs the operations here rather than asking a host.
 *
 * The fallback for a caller that can neither reach a host nor start one - a
 * sandbox that will not let a process be spawned, say. Every operation still
 * works; what is lost is the machine between commands, because there is nothing
 * left running to hold one. So a machine held for the length of one command is
 * what this offers, and `release` and `stop` are answered as the no-ops they
 * honestly are.
 *
 * The holder is built by the caller, so this module keeps importing nothing
 * that boots a machine.
 */
export function inProcessClient(holder: {
  call(
    operation: string,
    input: unknown,
  ): Promise<{
  ...
  return {
    call: async (operation, input) => {
      const { outcome, notes, failed } = await holder.call(operation, input);
      return { value: outcome, notes, failed };
    },
    ask: async (action) => {
      if (action === 'release') {
      ...
      if (action === 'stop') return { stopping: false };
      return { serving: ['ops'], holding: await holder.held() };
    },
    serving: () => ['ops'],
    close: () => void holder.dispose(),
  };
}
