...
describe('serializeProjectZip / parseProjectZip round-trip', () => {
  ...
  it('writes a code block asmSource as its own .asm entry', () => {
    const withAsm: Block = { ...BLOCK_B, asmSource: 'start:\n  RET\n' };
  ...
  it('round-trips the optional entry address and omits it when absent', () => {
    const withEntry: Block = { ...BLOCK_B, entry: 0x9010 };
    const parsed = parseProjectZip(
  ...
  it('round-trips the optional asmSource and omits it when absent', () => {
    ...
    const withAsm: Block = { ...BLOCK_B, asmSource: source };
...
describe('parseProjectZip error handling', () => {
  ...
  // A block was recorded as `kind: 'data'` back when that word meant memory
  // rather than a file a program saved. Nothing writes it now, so it can only
  // ever have meant memory - and an old project must open unchanged.
  it("reads a block stored as 'data' as a memory block", () => {
    const zip = zipWithMeta(
      {
        ...validMeta,
        blocks: [
          {
            ...
            address: 0x8000,
            kind: 'data',
            bin: 'blocks/SPRITES.bin',
          },
        ],
        listingBlockMeta: { 0: { kind: 'data' } },
      },
      { 'blocks/SPRITES.bin': 'ab' },
    );
    const parsed = parseProjectZip(zip);
    ...
    expect(Array.from(parsed.blocks[0]!.bytes)).toEqual([0x61, 0x62]);
    expect(parsed.listingBlockMeta[0]).toEqual({ kind: 'memory' });
  });

  it("reads an autosave-shape block stored as 'data' as a memory block", () => {
  ...
  it("never writes 'data' back for a block it read as one", () => {
    const [block] = parseBlocks([
      { id: 'b', name: 'B', address: 0, bytes: 'AQ==', kind: 'data' },
    ]);
    expect(serializeBlocks([block!])[0]!.kind).toBe('memory');
  });
  ...
  it("throws when a block's referenced .bin entry is missing", () => {
    const meta = {
      ...validMeta,
      blocks: [{ name: 'B', address: 0, kind: 'memory', bin: 'blocks/B.bin' }],
    };
  ...
  it('throws when a block name is invalid', () => {
    const meta = {
      ...validMeta,
      blocks: [
        { name: '1foo', address: 0, kind: 'memory', bin: 'blocks/1foo.bin' },
      ],
    };
    expect(() =>
  ...
  it('throws when two blocks share a name', () => {
    const meta = {
      ...validMeta,
      blocks: [
        { name: 'SAME', address: 0, kind: 'memory', bin: 'blocks/SAME.bin' },
        { name: 'SAME', address: 1, kind: 'memory', bin: 'blocks/SAME.bin' },
      ],
    };
    expect(() =>
