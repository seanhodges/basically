...
describe('Spectrum128Memory activity recording', () => {
  ...
  it('stamps read and write bits by CPU address when enabled', () => {
    const mem = new Spectrum128Memory(makeRom());
    mem.activity.enabled = true;
    mem.read(0x8000); // bank 2 read
    mem.write(0xc000, 0x42); // paged-window write
    mem.read(0xc000); // read the address it just wrote
    mem.read(0x0000); // ROM read still stamps

    const hits = mem.activity.hits;
    expect(hits[0x8000]).toBe(READ_BIT);
    expect(hits[0xc000]).toBe(READ_BIT | WRITE_BIT);
    expect(hits[0x0000]).toBe(READ_BIT);
    // An untouched address stays clear.
    expect(hits[0x9000]).toBe(0);
  });
