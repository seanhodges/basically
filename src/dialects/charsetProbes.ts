 ...
 *
 * Three consumers read this table, which is the reason it exists: the escape
 * scaffolder (`scripts/gen-escape-scaffold.mts`), the escape cross-check
 * (`docs/reference/data/escapes/escape-crosscheck.test.ts`), and the
 * semigraphics audit ({@link ./semigraphicsAudit}). They each used to carry
 * their own copy, which is how the scaffolder's Commodore entry drifted into
 * naming a file and a machine list the docs no longer used.
 *
