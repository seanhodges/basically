...
/** A small but representative .P image to round-trip through the tape codec. */
...
describe('decodeCassette', () => {
  it('round-trips name and .P bytes through encode→decode', () => {
    const image = sampleImage('10 PRINT "HELLO"\n20 GOTO 10\n');
    const samples = encodeCassette('GREETING', image, { sampleRate: RATE });
  ...
  it('recovers the original source via parsePFile + detokenize', () => {
    ...
    const samples = encodeCassette('PROG', image, { sampleRate: RATE });
    const { data } = decodeCassette(samples, RATE);
    const source = detokenizeProgram(parsePFile(data).program);
    expect(normalize(source)).toBe(normalize(src));
  });

  const image = sampleImage('10 PRINT "ROBUST"\n20 GOTO 10\n');
  const clean = encodeCassette('TAPE', image, { sampleRate: RATE });
