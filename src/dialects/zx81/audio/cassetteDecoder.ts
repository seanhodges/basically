 ...
 *
 * The encoder writes each byte MSB-first as a sequence of bits, where a '0' bit
 * is a burst of 4 carrier pulses and a '1' bit is a burst of 9; one pulse is
 * ~150µs high then ~150µs low (300µs period). Every bit is followed by a
 * silence gap, and a few seconds of silence lead in/out. The program name (ZX81
 * charset, last char +0x80) precedes the .P data.
 *
 * We recover the bits by gating the signal into tone bursts and **counting the
 * carrier half-cycles in each burst**, not by measuring absolute durations: only
 * the 4-vs-9 ratio distinguishes a bit, and that ratio survives playback/clock
 * speed drift, resampling and sample-rate mismatch — the dominant real-world
 * failure modes. The 4-vs-9 gap (≈7 vs ≈17 Schmitt transitions) leaves a wide
 * classification margin, so a miscount of a cycle or two never flips a bit.
 */
...
export function decodeCassette(
  ...
  if (peak < 1e-4 || range < peak * 0.5) throw new Error(NO_SIGNAL);
  ...
  if (bursts.length < 8) throw new Error(NO_SIGNAL);
  ...
  if (noisy > bursts.length * 0.2) throw new Error(NO_SIGNAL);

  // 6. Pack bits MSB-first into bytes (drop any trailing partial byte).
  const bytes = packBits(bits);
  if (bytes.length === 0) throw new Error(NO_SIGNAL);

  // 7. Split the name (ends at the first byte with bit 7 set) from the .P image.
