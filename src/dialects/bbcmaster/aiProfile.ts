...
- MODE 7 only: teletext control characters - PRINT CHR$(129);"red" (129-135 = red,green,yellow,blue,magenta,cyan,white), CHR$(141) double height (print the line twice), CHR$(136) flash. CHR$(128)-CHR$(159) are teletext controls that do nothing useful in graphics modes (0-6) - there, set colour with COLOUR/GCOL instead. In this IDE the same bytes can be written INSIDE string literals as named escapes - PRINT "{RED}red {FLASH}flash" or "{DOUBLE HEIGHT}big", plus {GRAPHICS GREEN} etc. and {0xNN} for any raw byte - which import/export byte-exactly; prefer them over CHR$(…) chains inside strings.
