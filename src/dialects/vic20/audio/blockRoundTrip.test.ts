...
/**
 * End-to-end cassette round trip for a VIC-20 document that carries memory
 * blocks: encode to tape audio with the dialect's `buildSamples`, decode it back
 * through `decodeSamples`, and confirm the main program, the CODE blocks and
 * (with an auto-loader) the preserved loader all survive - the VIC-20 counterpart
 * to the C64's `audio/blockRoundTrip.test.ts`.
 */
const audio = vic20.audio!;
...
describe('VIC-20 cassette round trip with memory blocks', () => {
  it('recovers the program and blocks (loader off)', () => {
    ...
    expect(blocks.map((b) => b.address)).toEqual([0x1b00, 0x1c00]);
