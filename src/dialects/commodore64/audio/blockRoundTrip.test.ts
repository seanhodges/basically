...
/**
 * End-to-end cassette round trip for a document that carries memory blocks:
 * encode to tape audio with {@link buildCassetteSamples}, decode it back through
 * the dialect's `decodeSamples`, and confirm the main program, the CODE blocks
 * and (with an auto-loader) the preserved loader all survive. This is what
 * "cassette/.wav export is compatible with multi-blocks" means on the import
 * side - before this the decode kept only the first program and dropped blocks.
 */
...
// A main program comfortably larger than the generated auto-loader (which grows
// with the block count), so the "largest BASIC program is the source" rule picks
// it and not the loader.
const source =
...
describe('C64 cassette round trip with memory blocks', () => {
  it('recovers the program and blocks (loader off)', () => {
    ...
    expect(blocks.map((b) => b.address)).toEqual([0xc000, 0xd000]);
