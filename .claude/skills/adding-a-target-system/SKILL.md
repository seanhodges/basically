...
| **5 — Memory map & runtime introspection** | `memoryMap.ts`, `memoryBlocks.ts` (+ machine-side block load/inject support in `loadProgram`), `sysvars.ts`/`vars.ts`/`reports.ts` → `readVariables`/`readReport`, `readMemoryStats` (spanning **every** pool a program spends — see the run-measurement rules below) + memory-activity hooks.                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | stages 2–3                                       | memory-map + blocks tests; watcher shows live vars                                       |
