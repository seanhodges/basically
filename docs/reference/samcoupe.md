...
It looks Sinclair-ish and is not: SAM BASIC is Andy Wright's Beta BASIC line,
with its own token table, line format and line-number ceiling. A ZX Spectrum
program has to be run through MGT's `BTRANS` utility to become a SAM one.
