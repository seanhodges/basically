...
Every escape that can be typed in Applesoft source, and the byte it stores. The
...
in a data block instead — or printed with `INVERSE` and `FLASH`, which is what
this BASIC has and the Apple II's has not. Filters can be prefilled with `?q=`
and `?cat=` query parameters.

See also the [Applesoft BASIC reference](../applesoft) and
[file formats](../file-formats#escape-notation).

<EscapeTable :data="applesoftEscapes" />
