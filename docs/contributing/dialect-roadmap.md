...
| --------------------------------- | ------------ | ---------------- | ------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- |
| Z80.js (Molly Howell)             | Z80          | MIT              | `src/emulator/z80/` - vendored, 3 local patches (M1 hook, accessors, ESM export)                  | ZX81, ZX80, ZX Spectrum 48K, ZX Spectrum 128, Amstrad CPC 464 |
...
pattern has now been used five times (`bbcmaster`, `zxspectrum128`, `vic20`,
`pet`, and the `cpc6128` scaffold), so **always check for a shipped sibling
before planning a new dialect**.
...
| ⬜     | Amstrad CPC 664               | Z80  | Locomotive BASIC 1.1 | Strictly **downstream of `cpc6128`** - it needs that plan's BASIC 1.1 keyword table and sysvars first, after which it is nearly free. The hardware delta from the 464 is small: 64K flat RAM (no `&7Fxx` banking, so none of the 6128's Stage B work), firmware v1.1, built-in 3" drive. Implementation is extending `CpcModel` in `src/emulator/cpc/memory.ts` from `'464' \| '6128'` with `'664'`. Blocked on sourcing a `cpc/cpc664.rom` image - do not commit a fabricated one (the `cpc6128` plan sets that precedent). |
