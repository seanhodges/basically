...
Let a held machine's memory be watched while it works. A caller that holds a
machine may ask for a map of its memory and is given an address anything that
shows a web page can show, so a person supervising an agent, or an application
embedding the toolchain in a frame of its own, can see the machine's layout and
the addresses its processor is touching as a program runs. A map mirrors and
never drives: it reports which addresses were touched and never what they hold,
the machine advances only when a request asks it to, and measurements are
