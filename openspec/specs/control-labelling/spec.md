# control-labelling Specification

## Purpose

What a control tells the user about itself: every control names its action
rather than its category, its hover tooltip and its name for assistive
technology agree, and a label reporting a keyboard shortcut or the supported
machines tells the truth. This holds across the toolbar, the tab bar, the
memory map, the dialogs and the standalone player alike.
