...
- [ ] 1.1 `src/reference/escape-classes.ts` (new): the closed vocabulary of what a
...
- [ ] 1.2 `src/reference/types.ts`: `EscapeTableData.categories` entries gain the
      class they belong to, documented as the cross-page handle for guidance while
      the id and label stay page-scoped.
- [ ] 1.3 `src/reference/escapes/*.ts`: classify every category on all nine pages.
      No row changes.
- [ ] 1.4 `src/reference/escapes/escape-data.test.ts`: every category declares a
...
- [ ] 2.1 `src/reference/escape-guidance.ts` (new): per (target page, class) —
...
- [ ] 2.2 Author the cells the crosscheck in 2.3 demands, starting with the
...
- [ ] 2.3 `src/reference/escape-guidance-crosscheck.test.ts` (new): walk every
...
- [ ] 3.1 `src/reference/compare.ts`: `EscapeSection` gains the group's class and
      ...
      order is unchanged.
- [ ] 3.2 `src/reference/compare.test.ts`: sections carry their class; the
...
- [ ] 3.3 `docs/.vitepress/theme/components/DialectCompare.vue`: render the
...
- [ ] 4.5 `npm run e2e:chromium -- e2e/porting-guidance` — extend an existing
