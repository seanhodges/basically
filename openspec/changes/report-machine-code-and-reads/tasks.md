...
- [ ] 1.1 `src/dialects/types.ts`: beside the memory-write declaration, an
...
- [ ] 1.2 Per-dialect declarations for every machine that has the syntax,
      each beside its existing write declaration.

## 2. Reading the program

- [ ] 2.1 `src/editor/pokeAddresses.ts` + its test: the read scan and the call
...
- [ ] 2.2 `src/app/programVocabulary.ts`: read sites and call sites join the
      ...
      payloads.
- [ ] 2.3 `src/app/programVocabulary.test.ts`: a PEEK is collected with its
...
- [ ] 2.4 `src/components/DocsDrawer.tsx` + `DocsDrawer.test.ts`: the new
      fields cross the boundary and the field lists agree by string.

## 3. The table entries

- [ ] 3.1 `src/reference/porting.ts` + its crosschecks: the run-a-routine
...
- [ ] 4.1 `src/reference/compare.ts` + `compare.test.ts`: read landings —
...
- [ ] 4.2 Same files: the machine-code finding — call sites and blocks
      gathered, calls resolved into the blocks that contain them, produced
      whenever the program has either.
- [ ] 4.3 `src/reference/portDescription.ts` + its test: read landings beside
...
- [ ] 4.4 `docs/.vitepress/theme/components/DialectCompare.vue`: both findings
      rendered with the rewrites; the layout marks include read addresses.
- [ ] 4.5 `src/ai/portReport.ts` + its test: both findings join the hand-over.
