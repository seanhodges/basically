...
- [ ] 1.1 Add `src/editor/tokenAt.ts`: given editor state and a document position,
...
- [ ] 1.2 Add `src/editor/tokenAt.test.ts` on a crunched, an Acorn and a Sinclair
...
- [ ] 1.3 Extend that test with the lazy-parse guard: a document of several thousand
      ...
      menu row.
- [ ] 1.4 Extend that test for the assembly tokenizer, per processor: an instruction and
...
- [ ] 2.1 Add `src/editor/clickMenu.ts` holding the reusable half of
...
- [ ] 2.2 Give the menu its own Escape binding: handled only while a menu is open, and
...
- [ ] 2.3 Reduce `src/editor/variableUsagesView.ts` to the usages half — the usages
...
- [ ] 2.4 Confirm the refactor is behaviour-preserving: `npm test` and
...
- [ ] 3.1 Add `src/editor/referenceRow.ts`: a token reader that layers the
...
- [ ] 3.2 Skip single-character tokens drawn from the highlighter's punctuation set —
      ...
      restating it.
- [ ] 3.3 Add `src/editor/referenceRow.test.ts`: each punctuation character is skipped
...
- [ ] 4.1 `src/components/CodeMirrorHost.tsx`: replace the usages extension with the
...
- [ ] 4.2 `src/components/AsmEditor.tsx`: mount the menu with the reference row alone,
...
- [ ] 5.1 `src/app/docsTopic.ts`: delete the first-word-of-a-selection helper; the topic
      ...
      selection.
- [ ] 5.2 `src/app/store.ts`: remove the mirrored-selection field, its doc comment, its
      ...
      property.
- [ ] 5.3 Rewrite the stale comments on the three openers in `src/components/Toolbar.tsx`
...
- [ ] 5.4 Update `src/app/docsTopic.test.ts`: drop the selection field from every state
...
- [ ] 5.5 Check `src/app/store.test.ts`, `src/app/surfaces.test.ts` and
      `src/app/historyNav.test.ts` for fallout from the removed store field.

## 6. Styling

- [ ] 6.1 `src/styles.css`: rename the menu's own classes away from the usages feature
...
- [ ] 6.2 Replace the hardcoded row glyph with per-row icon modifiers: `⌕` for usages,
      `?` for reference. Note in the comment that the list is built for N rows even
      though only one is ever offered today.

## 7. Documentation

- [ ] 7.1 `docs/guide/keyboard-shortcuts.md`: the sentence stating that F1 jumps to the
      selected keyword is now false — replace it with a pointer to the editor menu.
- [ ] 7.2 `docs/guide/writing-basic.md`: add the reference gesture beside the existing
...
- [ ] 8.1 Extend the existing journey in `e2e/code-editor/variable-usages.spec.ts` with
...
- [ ] 8.2 Rewrite the frame-routing test in `e2e/shell/docs-drawer.spec.ts` to drive from
...
- [ ] 8.3 Extend the existing journey in `e2e/memory-blocks/asm-editor.spec.ts` (its
...
- [ ] 9.4 `npm run docs:build` (this change edits `docs/`)
- [ ] 9.5 `npm run e2e:chromium -- e2e/code-editor`
- [ ] 9.6 `npm run e2e:chromium -- e2e/shell`
- [ ] 9.7 `npm run e2e:chromium -- e2e/memory-blocks`
- [ ] 9.8 `npx openspec validate --specs`
- [ ] 9.9 By hand (`npm run dev`): click a keyword → Reference → the drawer opens seeded;
      click a variable → Usages still works; click a comma → no menu; open a machine-code
      block and click an instruction → the processor page seeded; F1 with nothing clicked
      → docs home; switch machine keeping the program → F1 still lands on the comparison,
      but a keyword click still goes to the keyword. Spot-check an abbreviated spelling in
      a pasted Commodore listing, and the menu on a touch viewport.
