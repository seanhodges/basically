...
- [x] 1.1 Measure what one frame costs to carry at full rate for the widest
      ...
      later.
- [x] 1.2 Decide from 1.1 whether full frames hold at the machine's own rate or
      ...
      Questions.
- [x] 1.3 Confirm the WebSocket handshake and framing are worth hand-rolling
...
- [x] 2.1 Admit the operations conversation to the entry point that serves a
      conversation over the process's own streams, which is limited today to the
      other two by nothing but its own signature.
- [x] 2.2 Accept it in the argument parser alongside the operations that serve an
      editor and an agent, and reject the same malformed forms they reject.
- [x] 2.3 Add its usage text beside theirs, saying how an application is expected
      to start it.
- [x] 2.4 Colocated tests: the conversation is served over the streams, a caller
...
- [x] 3.1 Add a self-correcting pacing loop beside the machine — in its worker,
...
- [x] 3.2 Start and stop that loop with the play channel, so a machine nobody is
      playing advances exactly as it does today.
- [x] 3.3 Colocated tests: a machine whose rate is not a round number keeps time;
...
- [x] 4.1 Extend the seam between a caller's machine and its projection so keys
...
- [x] 4.2 Serve the channel from the host's listener, reusing the loopback
...
- [x] 5.1 Write the page the channel is played through as a sibling to the view's
...
- [x] 5.2 Map the browser's key codes onto the machine's key names in the page,
      so the host never sees a browser event.
- [x] 5.3 Say what the channel is showing — a machine being played, no machine
      up, or a machine that has gone — so a still picture is never ambiguous.
- [x] 5.4 Size the machine's picture to whatever frame the embedding application
      gives it, at its own aspect ratio, as the view's page does.

## 6. Playing as an operation, and what it costs other requests

- [x] 6.1 Declare playing as one operation, answering with an address, whether
...
- [x] 6.2 Refuse the holder's requests that act on or measure a machine being
      ...
      costs.
- [x] 6.3 Let a caller give up its play channel without giving up its machine.
- [x] 6.4 Colocated tests: a measurement of a played machine is refused with the
...
- [x] 7.1 End a view when the caller asks to play the same machine, and end a
...
- [x] 7.2 Colocated tests: each direction ends the other and says so, and the
      ended projection's address admits nothing afterwards.

## 8. Documentation

- [x] 8.1 Document playing beside watching, saying in its own words — not by
      reference to the view's — that possession of the address admits acting on
      the machine rather than watching it.
- [x] 8.2 Document the operations conversation over standard streams beside the
...
- [x] 9.2 `npx vitest run src/server/ src/ops/ src/cli/ src/dialects/headless/` —
...
- [x] 9.6 No e2e run applies. This change is toolchain-only and adds nothing to
