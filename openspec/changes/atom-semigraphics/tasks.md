...
- [x] 1.1 In `src/dialects/atom/charset.ts`, add a `sextantChar(code)` that
      ...
      six-bit reversal.
- [x] 1.2 Wire it into `decodeSpan` (graphics bytes decode to their sextant
...
- [x] 1.3 Extend the charset tests (`src/dialects/atom/tokenizer.test.ts` holds
...
- [x] 1.4 Confirm `src/dialects/atom/tokenizer.ts` handles a sextant in a
...
- [x] 2.1 New `src/dialects/atom/semigraphics.test.ts`: crosscheck against
...
- [x] 2.2 In the same file, a kernel-ROM probe test against the real Atom ROM
...
- [x] 3.1 New `src/dialects/atom/graphics.ts`: one `GraphicEntry[]` section for
      ...
      thing to teach.
- [x] 3.2 Add a `palette: 'graphics'` editor mode and the `graphicsPalette` to
      `src/dialects/atom/keyboardLayout.ts`, alongside the existing ABC / SYM /
      CURSOR modes.
- [x] 3.3 Extend `src/dialects/atom/keyboardLayout.test.ts`: every palette
      `char` re-encodes through `atomCharset.toMachine` to its declared `code`.

## 4. Audit and round-trip guarantee

- [x] 4.1 In `src/dialects/semigraphicsAudit.ts`, replace
      ...
      `IN_SCOPE`.
- [x] 4.2 Add `atom` to `e2e/paletteMachines.ts` in registry order (between
      `vic20` and `trs80`), so `src/dialects/graphicsPalette.test.ts` passes.
- [x] 4.3 Add `atom` to `src/dialects/semigraphicsRoundTrip.test.ts` with target
      ...
      range entirely.
- [x] 4.4 Confirm no new code points: the sextants were all bundled for the
...
- [x] 5.1 Correct `docs/reference/data/escapes/atom.ts`: the inverse-video entry
...
- [x] 5.2 Run `npm run gen:semigraphics` and confirm
      `docs/contributing/semigraphics-support.test.ts` passes.
- [x] 5.3 Rewrite the support doc's hand-written prose: drop the "Machines this
...
- [x] 5.4 Note the sextant spelling and the palette in
      `src/dialects/atom/aiProfile.ts`, as the TRS-80 and BBC profiles do.
- [x] 5.5 Delete `docs/contributing/dialect-plans/atom-semigraphics.md` (the
...
- [x] 6.2 `npm run docs:build` (this change touches `docs/`)
- [x] 6.3 `npx openspec validate --specs`
- [x] 6.4 `npm run e2e:chromium -- e2e/virtual-input` — the palette gains a
