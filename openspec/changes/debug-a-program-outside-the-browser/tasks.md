...
- [ ] 1.1 Add the members that express stopping to the driver over one machine:
...
- [ ] 1.2 Take the machine's stopping path as something the holder hands in,
      ...
      stepped.
- [ ] 1.3 Keep the line the caller resumed from beside the driver and thread it
      through every slice, so continuing off a line that is itself a stop does
      not stop again at once.
- [ ] 1.4 Bound stepping and continuing with the frames bound the driver's waits
      already use rather than a new number, and report exhausting it as an
      ordinary outcome.
- [ ] 1.5 Report what each step and each continue cost in the machine's own
      emulated time.
- [ ] 1.6 Colocated tests against real ROMs: a program stops before a line it was
...
- [ ] 2.1 Hold the lines to stop before beside the machine the host holds for a
      caller, so they survive between requests as the machine does, and are that
      caller's alone.
- [ ] 2.2 Take the stopping path in the runner that holds a machine for every
...
- [ ] 2.3 Carry the requests across to a machine held in a worker and the answers
      back, on the arrangement that already carries a key toward one.
- [ ] 2.4 Colocated tests: the lines in force survive between requests; two
...
- [ ] 3.1 Accept the lines to stop before as part of what a run is asked for, and
      have the run take the stopping path when any is named.
- [ ] 3.2 Report a run that stopped distinguishably from one that ended and one
      that used up its frames, naming the line it stopped before, and leave the
      machine there.
- [ ] 3.3 Refuse a run told where to stop on a machine whose ROM is absent, as
      the caller's mistake, with the remedy the schedule's refusal already names.
- [ ] 3.4 Colocated tests: a run stops before the line it was told to; a run
...
- [ ] 4.1 Declare naming the lines to stop before, stepping, continuing and
      asking where — one operation each, each with what it needs, what it answers
      and what it does to a machine being played.
- [ ] 4.2 Declare each one's absence from the assistant with its own reason,
...
- [ ] 4.3 Route all four to the command line and to an agent, and check the
      parity test passes with the declared exemptions in both directions.
- [ ] 4.4 Colocated tests: each operation answers a caller holding no machine by
...
- [ ] 5.1 Report whether a machine can be stepped where a machine is described,
      read from what that machine declares rather than from a list beside it.
- [ ] 5.2 Answer a caller that asks a machine which cannot be stepped to stop,
      step or continue by saying so and naming what it can still do.
- [ ] 5.3 Colocated tests: every registered machine's description agrees with
...
- [ ] 6.1 Refuse naming the lines to stop before, stepping and continuing while a
...
- [ ] 6.2 Colocated tests: each of the three is refused with the remedy while a
      machine is played; asking where is answered; and all three are answered
      again the moment the channel ends.

## 7. The command line

- [ ] 7.1 Accept the four operations in the argument parser, and the lines to stop
      before as an option on a run, rejecting the malformed forms the neighbouring
      operations reject.
- [ ] 7.2 Render each outcome as columns and prose the way the held-machine
      operations beside them are rendered, and add each one's usage text.
- [ ] 7.3 Colocated tests: the grammar accepts each operation and the run option;
      a malformed list of lines is the caller's mistake; and the usage text names
      the operations and the option.

## 8. Documentation

- [ ] 8.1 Document debugging a program outside the browser, beside the pages for
...
- [ ] 8.2 Say in the page for an embedding application that these operations are
...
- [ ] 9.2 `npx vitest run src/app/ src/ops/ src/mcp/ src/server/ src/cli/
...
- [ ] 9.6 `npm run e2e:chromium -- e2e/program-execution` — not because this
