...
- [ ] 1.1 Add a scratch-buffer list to the store: id, display name, text, and its
      own breakpoint set. Session-only state — it goes nowhere near the autosave
      keys or the project serializer.
- [ ] 1.2 Generalise the active tab from a nullable block id to a three-way
      ...
      the removed ordinal.
- [ ] 1.3 Actions to add, rename, retext and close a scratch buffer. Names are
...
- [ ] 1.4 Lifecycle: scratch buffers are cleared on a target-machine switch and
      ...
      became active" path.
- [ ] 1.5 Colocated store tests: editing a scratch buffer leaves `source`,
...
- [ ] 2.1 A selector for the active buffer's breakpoint set, in the shape of the
      ...
      buffer on screen.
- [ ] 2.2 Toggle and clear act on the active buffer. The document's set stays
...
- [ ] 2.3 Pin a running debug session to the buffer that started it. The debug
...
- [ ] 2.4 Track which buffer a pause belongs to, and show the paused-line
      highlight and the "paused at line N" status only while that buffer is the
      one on screen.
- [ ] 2.5 Colocated tests: toggling on a scratch leaves the program's set alone
...
- [ ] 3.1 Route the editor's document-push and change handler by the active tab
...
- [ ] 3.2 A tab switch pushes the incoming buffer's text through the editor's
      existing `{text, seq}` channel. Verify the switch cannot lose the last
      keystroke of the outgoing buffer.
- [ ] 3.3 Re-check the singleton behaviours that survive on one mounted editor:
...
- [ ] 4.1 The run path takes its source from the active buffer — the scratch's
      text when one is showing, else `source`. The assistant's answer-check keeps
      precedence and is untouched.
- [ ] 4.2 Bypass the preserved-boot-disc short circuit for a scratch run.
...
- [ ] 4.3 A scratch run carries the document's memory blocks — block linting uses
      ...
      boot-disc image.
- [ ] 4.4 Colocated tests for run-source selection: scratch active → the
      scratch's text; BASIC or a block tab active → `source`; an assistant check
      still wins over both.

## 5. The tab strip

- [ ] 5.1 Render scratch tabs after the block tabs, with a glyph distinct from
      the code and data block glyphs so they read as not part of the document.
- [ ] 5.2 The strip currently returns nothing for a dialect with no memory-block
      support — render it unconditionally, since scratch tabs are
      dialect-independent.
- [ ] 5.3 Turn the `+` button into a two-item menu (new scratch buffer / new
      machine code block), reusing the dismiss and context-menu pieces the strip
      already imports.
- [ ] 5.4 A scratch tab's context menu (right-click and long-press, as the block
      tabs have): rename, download as `.bas`, close. No confirmation on close.
- [ ] 5.5 Name the buffer the run control will run, so it is clear before
      pressing it that a snippet and not the program is about to boot.

## 6. Readers that follow the buffer on screen

- [ ] 6.1 Point the status-bar size and error counts, the memory map, the
      procedure outline and the contextual documentation at the active buffer.
- [ ] 6.2 Leave hardware export, the assistant's source and staleness base,
...
- [ ] 7.1 One e2e journey in `e2e/code-editor/`: create a scratch buffer, type a
...
- [ ] 7.2 `npm run typecheck && npm test && npm run lint && npm run format:check`
- [ ] 7.3 `npm run e2e:chromium -- e2e/code-editor e2e/program-execution` —
