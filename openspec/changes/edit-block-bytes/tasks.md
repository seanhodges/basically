...
- [x] 1.1 New pure projection module: bytes + view mode (hex, characters, both) +
...
- [x] 1.2 New pure module for byte editing — apply a hex nibble (high then low,
...
- [x] 1.3 Colocated tests for both. Projection: offset↔byte round-trips in every
...
- [x] 2.1 New byte-editor component on CodeMirror, as `AsmEditor` is: the
...
- [x] 2.2 Take ordinary text input out of the document so the surface's own
...
- [x] 2.3 Editing in both views: hex digits in the hex view with any other key
...
- [x] 2.4 Undo through `src/editor/bufferHistory.ts` under `blockBufferKey`, as
      ...
      was put away with.
- [x] 2.5 On-screen keyboard: install this surface's applier into
...
- [x] 2.6 Responsive views: both side by side where there is room, tabs where
...
- [x] 3.1 `src/components/Workspace.tsx` — route a data block, and a code block
      ...
      now owns.
- [x] 3.2 Commit fork: `upsertBlock` for an ordinary block,
      ...
      the user.
- [x] 3.3 The byte count in the surface's status strip is an editable field
      ...
      growth path.
- [x] 3.4 `src/components/EditorTabBar.tsx` — a "Load bytes…" entry in the block
...
- [x] 4.1 The unit tests from group 1 are the bulk of the coverage — the
...
- [x] 4.2 Replace `e2e/memory-blocks/asm-editor.spec.ts`'s "a data block shows
...
- [x] 4.3 One e2e scenario in `e2e/memory-blocks/`, extending an existing journey
      ...
      real browser.
- [x] 4.4 Extend `e2e/memory-blocks/zx81-listing-blocks.spec.ts` for a listing
      block switched to data: this is the case where the commit path differs and
      the BASIC listing is rewritten underneath.
- [x] 4.5 Do **not** add a per-machine e2e matrix for the byte editor. It is
      dialect-generic apart from the charset, and the charset behaviour is a unit
      test.

## 5. Documentation

- [x] 5.1 `docs/guide/machine-code.md` — the sentence "a **data** block is a
...
- [x] 6.2 `npm run docs:build` (docs/ changed in group 5).
- [x] 6.3 `npm run e2e:chromium -- e2e/memory-blocks` and
      ...
      what failed.
- [x] 6.4 Phone-portrait viewport, driven through a throwaway Chromium spec
      rather than by hand (no device in this environment): the views drop to
      Hex / Characters tabs, the row drops to 8 bytes, the on-screen keyboard
      reaches the surface (`A` then `9` writes `A9`, `Z` is ignored in the hex
      view), and the content carries `inputmode="none"` — so the device
      keyboard will not appear, which is intended. Still worth a look on real
