...
- [x] 1.1 Add `src/reference/pages.ts`: hand-authored page-id → reference
...
- [x] 1.2 Switch the modules and tests that recompute `docsReference ?? id`
      ...
      `referencePageOf`
- [x] 1.3 Replace the eight test-local page tables
...
- [x] 2.1 Rework `src/editor/constructs.test.ts` to iterate the registry
...
- [x] 2.2 Add registry-driven coverage for `src/editor/variableLint.ts`
      wrappers with a crosschecked exemption map
- [x] 2.3 Pin `src/reference/porting.ts` membership (every reference page
...
- [x] 4.2 Add a test that `docs/.vitepress/config.ts` sidebar and
      ...
      exposes as missing)
- [x] 4.3 Move the `MACHINES` list from
...
- [x] 5.1 Deliberate-failure pass: for each new pin, remove one machine's
...
- [x] 5.3 `npm run e2e:chromium -- e2e/program-execution` (the boot spec
