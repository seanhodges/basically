...
- [x] 1.1 `src/reference/types.ts` + `src/reference/facts.ts`: every machine
...
- [x] 1.2 `src/reference/facts-crosscheck.test.ts`: the structured geometry
      agrees with the columns-by-rows figure the prose states; the clock idiom
      answers to the machine's own reference rows.
- [x] 1.3 A registry-driven benchmark test: boot every registered machine, run
...
- [x] 2.1 `src/app/programVocabulary.ts`: constant position arguments —
...
- [x] 2.2 `src/app/programVocabulary.test.ts`: positions collected per form;
...
- [x] 2.3 `src/components/DocsDrawer.tsx` + `DocsDrawer.test.ts`: the new
      fields cross the boundary and the field lists agree by string.

## 3. The findings

- [x] 3.1 `src/reference/compare.ts` + `compare.test.ts`: the positions
...
- [x] 4.1 `src/reference/portDescription.ts` + its test: positions among the
...
- [x] 4.2 `docs/.vitepress/theme/components/DialectCompare.vue`: the findings
      rendered under the same conditions.
- [x] 4.3 `src/ai/portReport.ts` + its test: all three join the hand-over.
