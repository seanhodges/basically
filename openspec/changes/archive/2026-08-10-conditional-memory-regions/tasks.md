...
- [x] 1.1 `src/app/programVocabulary.ts`: the vocabulary reports the screen
...
- [x] 2.1 `src/dialects/types.ts`: the memory-blocks support contract gains
      ...
      untouched.
- [x] 2.2 `src/dialects/atom/memoryBlocks.ts` + colocated test: the video RAM
...
- [x] 2.3 `src/dialects/bbcmicro/memoryBlocks.ts`,
...
- [x] 3.1 `src/app/blockLint.ts`: the linter optionally takes the open
...
- [x] 3.2 Same file: the BBC's blanket screen-band warning is not emitted for a
...
- [x] 3.3 `src/app/blockLint.test.ts`: the met / unmet / computed-mode /
...
- [x] 3.4 `src/components/EmulatorPane.tsx`: the run gate hands the linter the
      vocabulary it already computes.

## 4. The porting comparison

- [x] 4.1 `src/reference/types.ts` + `src/reference/facts.ts`: the regions
      restated as reference data — start, end, bytes, condition, note — for the
      Atom, BBC Micro and BBC Master.
- [x] 4.2 `src/reference/facts-crosscheck.test.ts`: the restated regions agree
      byte-for-byte with the dialect declarations, condition included.
- [x] 4.3 `src/reference/compare.ts` + `compare.test.ts`: a pure condition
      ...
      condition is met.
- [x] 4.4 `src/reference/portDescription.ts` + its test: the finding rendered
      ...
      program.
- [x] 4.5 `docs/.vitepress/theme/components/DialectCompare.vue`: the finding
      inside the existing fit block, under the same gate.
- [x] 4.6 `src/ai/portReport.ts` + its test: the finding joins the hand-over
...
- [x] 5.5 `npm run e2e:chromium -- e2e/memory-blocks` — extend an existing
...
- [x] 5.6 `npm run e2e:chromium -- e2e/porting-guidance` — the fit report of a
