...
- [x] 1.1 Add the ` ```basic-expect ` grammar in a new `src/ai/expectations.ts`:
...
- [x] 1.2 Teach `src/ai/codeExtractor.ts` to recognise the tag, and pin in
...
- [x] 1.3 Check the apply paths (`AiPanel.applyText` and whatever selects the
...
- [x] 2.1 Add the evaluator to `src/ai/expectations.ts`: given the parsed
...
- [x] 2.2 Extend `src/app/aiRunCheck.ts` with the latch: an expectation that has
...
- [x] 3.1 Carry the expectations from the applied reply to the run: extract them
      in `aiStore` when applying, and hand them to the IDE store alongside the
      existing `aiRunCheckSeq` / run source.
- [x] 3.2 Evaluate them in `EmulatorPane`'s armed check - sampled every N frames
...
- [x] 3.3 In `aiStore`'s outcome subscription, treat a failed expectation as a
...
- [x] 3.4 Extend `buildRunNote` so a run whose expectations all held says so,
...
- [x] 4.1 Add the derived per-dialect capability statement (variables reportable
...
- [x] 4.2 Add the crosscheck test that constructs every registered machine and
...
- [x] 4.3 Add the expectation block to the returning-code rules in
...
- [x] 5.5 `npm run e2e:chromium -- e2e/ai-assistant` - the modified capability.
      Check off only when the run passes; on failure leave it unchecked with a
      note on what failed.
- [x] 5.6 `npm run e2e:chromium -- e2e/program-execution` - the run path the
