...
- [x] 1.1 Add `src/reference/portDescription.ts` with `PortSide` (extending
...
- [x] 1.2 Compute the diffs once at the top of `describePort`:
...
- [x] 1.3 Compose `PORTING THIS PROGRAM`: both machines with manufacturer and
...
- [x] 1.4 Compose `BEFORE YOU START` from `composeGuidance().pairNotes` followed
      by `.targetNotes`, unnarrowed (guidance prose states rules that hold for
      any program).
- [x] 1.5 Compose `SAME WORD, DIFFERENT MEANING` from the narrowed
      `FalseFriendWarning[]`, naming what the word means on each machine.
- [x] 1.6 Compose `COMMANDS THIS PROGRAM USES THAT <TARGET> DOES NOT HAVE` from
...
- [x] 1.7 Compose `COMMANDS TO RENAME` from `diff.renamed` (`FROM → TO`).
- [x] 1.8 Compose `COMMANDS WHOSE USAGE DIFFERS` from `diff.behaviourChanged`:
      the `parens` changes named together in one run, `kind` and `arguments`
      changes one line each giving both usages.
- [x] 1.9 Compose `CONTROL CODES THIS PROGRAM USES THAT <TARGET> DOES NOT HAVE`
...
- [x] 1.10 Join with `sections.filter((s) => s !== '').join('\n\n')` as
...
- [x] 2.1 Add `src/reference/portDescription.test.ts` covering, for real pairs
...
- [x] 2.5 A sweep over every ordered pair in `src/reference/machines.ts` that
...
- [x] 3.1 Add `ESCAPE_PAGES` to `src/ai/machineReference.ts` beside
...
- [x] 3.2 Export `loadReferencePage(page)` and `loadEscapePage(page)` from that
      ...
      degrade.
- [x] 3.3 Extend `src/ai/machineReference.test.ts`: every registered dialect's
      page resolves an escape table, or is named in an explicit list of pages
      that have none.
- [x] 3.4 Add `src/ai/portReport.ts` with `loadPortReport(from, to, vocabulary)`:
      ...
      module system.
- [x] 3.5 Add `buildConversionMessage({ from, to, toLabel, source })` to the same
...
- [x] 3.6 Write the two decline messages here, next to the decision that emits
...
- [x] 3.7 Add `src/ai/portReport.test.ts`: an empty source and an unreadable one
...
- [x] 4.4 Extend `src/components/DocsDrawer.test.ts`: the existing
...
- [x] 5.1 Rewrite `convertProgram` in `src/components/DocsDrawer.tsx` to resolve
      ...
      dynamic imports.
- [x] 5.2 On `ok: false`, call `setStatusNotice(message)` and return — before
...
- [x] 5.3 Keep the credentials check first, so a user with no assistant sees
...
- [x] 5.4 Comment recording that a wrongly guessed source machine is worse than
...
- [x] 6.1 Add a test to `e2e/porting-guidance/convert-program.spec.ts` that
...
- [x] 6.2 Assert in that same test that the turn does not carry what the
      Spectrum adds and the program never used.
- [x] 6.3 Add a test that asks to convert with the editor empty: no provider
      request is made, the status bar says there is nothing to convert, and the
      machine and the program are as they were.
- [x] 6.4 Add a test that asks to convert a program that cannot be read as the
...
- [x] 6.5 Re-run the three existing tests in that file unchanged — the switch,
...
- [x] 7.1 Extend the `src/reference/` and AI-path sections of
...
- [x] 8.5 `npm run docs:build` (`DialectCompare.vue` and
      `docs/contributing/architecture.md` both change)
- [x] 8.6 `npm run e2e:chromium -- e2e/porting-guidance`
- [x] 8.7 `npm run e2e:chromium -- e2e/ai-assistant`
