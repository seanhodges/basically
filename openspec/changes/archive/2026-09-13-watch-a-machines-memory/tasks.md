...
- [x] 1.1 Add an activity tap in `src/dialects/headless/`, beside `frameTap.ts`
...
- [x] 1.2 Arm and disarm the machine's memory-activity recording from whether
...
- [x] 1.3 Drain with the recycle buffer the seam already documents, so a mapped
      run does not allocate an address-space-sized buffer per sample.
- [x] 1.4 Reduce a drain to the coarsest grouping the picture can distinguish,
      ...
      display's.
- [x] 1.5 Colocated `activityTap.test.ts`: nothing recorded while unwatched;
...
- [x] 1.6 Fold the tap into the single per-frame step every path already folds
...
- [x] 2.1 An encoder modelled on the play channel's frame encoder: compress the
      reduced activity, and skip a payload identical to the one before it.
- [x] 2.2 Extend the machine thread's vocabulary with the map's open, activity
...
- [x] 2.3 Hold the tap off until the host says the last payload landed, so a
...
- [x] 3.1 A map channel beside `src/server/view/` and `src/server/play/`: the
      layout and bands sent once on arrival, activity thereafter, and the state
      the map is showing.
- [x] 3.2 Carry it over the WebSocket the play channel already serves, rather
      than a second carriage.
- [x] 3.3 Move the band-collapse transform out of the IDE's components to
...
- [x] 3.4 A standalone hand-written page, in the manner of the other two and
...
- [x] 3.5 Wire it into the projection host: its route, its own set of live
...
- [x] 3.6 Tests over the feed with nothing bound, in the manner of the view
      ...
      machine is let go.
- [x] 3.7 Host tests: reachable only from this computer, the address is the
...
- [x] 4.1 Declare the operation: it needs a session, it is answered while the
      machine is played, and its outcome is an address, whether one was already
      open, and why there is none when there is none.
- [x] 4.2 Add the projection to what an operation may be given, so a caller that
      cannot serve one carries none and is told so rather than failing.
- [x] 4.3 Register it, and add the action that gives a map up while keeping the
      machine - through the host protocol, the listener, the client and the
      command line's `--stop`.
- [x] 4.4 A written exemption for the one caller that deliberately lacks it, in
      its own words rather than borrowed from the view's, and the rows the
      surface-parity tables need.
- [x] 4.5 Tests: a machine with no described layout is refused and says why; a
...
- [x] 5.1 `docs/contributing/architecture.md`: the projections table becomes a
...
- [x] 5.2 The command-line reference for the new operation, and the page that
      covers watching a machine.

## 6. Quality gates

- [x] 6.1 `npm run typecheck`
- [x] 6.2 `npx vitest run src/ops/ src/server/ src/dialects/headless/ src/components/memoryActivity/ src/dialects/memoryActivity.test.ts src/dialects/memoryBands.test.ts`
- [x] 6.3 `npm run lint`
- [x] 6.4 `npm run format:check`
- [x] 6.5 `npm run docs:build`
- [x] 6.6 `npm run e2e:chromium -- e2e/memory-map`
