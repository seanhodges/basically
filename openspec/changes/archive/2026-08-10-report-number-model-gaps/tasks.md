...
- [x] 1.1 `src/reference/types.ts`: number handling gains an optional fractions
...
- [x] 1.2 `src/reference/facts.ts`: author the Atom's fractions alternative (the
...
- [x] 1.3 `src/reference/facts-crosscheck.test.ts`: a fractions alternative may
...
- [x] 2.1 `src/app/programVocabulary.ts`: the vocabulary reports the distinct
...
- [x] 2.2 `src/app/programVocabulary.test.ts`: the census collects a large
      literal, ignores one in a string, ignores small literals, and de-dupes.
- [x] 2.3 `src/components/DocsDrawer.tsx` + `DocsDrawer.test.ts`: the new field
      crosses the boundary and the two sides' field lists still agree by string.
- [x] 2.4 `src/reference/facts-crosscheck.test.ts` (or the census's own test):
...
- [x] 3.1 `src/reference/compare.ts`: a pure range-narrowing finding — both
...
- [x] 3.2 Same file: a pure marker-loss finding — markers the program's
...
- [x] 3.3 `src/reference/compare.test.ts`: Atom→ZX80 fires with and without
...
- [x] 4.1 `src/reference/portDescription.ts` + its test: the marker-loss section
...
- [x] 4.2 `docs/.vitepress/theme/components/DialectCompare.vue`: render both
      findings beside the findings they extend, under the same narrowing
      conditions.
- [x] 4.3 `src/ai/portReport.ts` + its test: both findings join what the
