...
- [x] 1.1 `treeCovering` takes the parse budget as an argument instead of reading
...
- [x] 1.2 The click path passes the interactive budget it has today, unchanged.
...
- [x] 1.3 The colour path asks for a complete parse. Say in a comment why that is
...
- [x] 2.1 `tokensIn` reports every token of the range asked about, for a program
      of any length. No caller of it needs to know a budget exists.
- [x] 2.2 Check whether the whole-program and the range answers can now share one
...
- [x] 3.1 Colocated test: a listing far longer than the old budget could cover is
...
- [x] 3.2 Colocated test: asking twice about an unchanged program yields the same
...
- [x] 3.3 Colocated test: a range at the end of a long program is reported in
      full, on the same terms as one at the start.
- [x] 3.4 The existing test that proves the walk reaches past the lazily-parsed
...
- [x] 3.5 The click path's tests are unchanged, and must stay passing untouched —
      that they need no edit is the evidence its behaviour was not disturbed.

## 4. Correct the withdrawn claim

- [x] 4.1 The design note shipped with `colour-a-program-by-its-machine` says a
...
- [x] 4.2 Check whether `docs/guide/language-server.md` or
...
- [x] 5.2 `npx vitest run src/editor/ src/lsp/`
- [x] 5.3 `npm test` — the shared editor plumbing is touched, and the last change
      to this exact helper went green locally and red on a CI shard.
- [x] 5.4 `npm run e2e:chromium -- e2e/code-editor` — the click path shares the
...
- [x] 5.5 `npm run docs:build`, only if 4.2 touched anything under `docs/`.
- [x] 5.6 Drive the built server end to end: open a listing far longer than the
