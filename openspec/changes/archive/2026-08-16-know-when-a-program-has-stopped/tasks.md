...
- [x] 1.1 Confirm `pause-and-continue-a-run` and
...
- [x] 2.1 **ZX81** (`src/dialects/zx81/emulator/zx81Machine.ts`): latch on
...
- [x] 2.2 **ZX80** (`src/dialects/zx80/emulator/zx80Machine.ts`): the same
...
- [x] 2.4 **ZX Spectrum 128K**: probe first. `0x1303` is expected to be the right
...
- [x] 2.5 **ZX Spectrum 128K** (`spectrum128Machine.ts`): implement what 2.4
      ...
      answers.
- [x] 2.6 **Acorn Atom** (`src/emulator/atom/atomMachine.ts`): latch on
...
- [x] 3.1 Extend `MachineEmulator.isProgramRunning()`'s documented contract with
...
- [x] 3.2 Add one registry-driven conformance test that boots every registered
...
- [x] 3.3 Add the companion case to the same test: a program that loops forever
      is still reported as running at the end of the window.
- [x] 3.4 Retire the five per-machine `isProgramRunning` traces that the shared
...
- [x] 4.1 Drop the `?` from `isProgramRunning` in `src/dialects/types.ts` and
...
- [x] 4.2 Remove the `undefined` case from `AiRunFrame.running` in
...
- [x] 4.3 Simplify `timingFrame()` in `src/app/runTiming.ts`: every machine now
