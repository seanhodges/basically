...
- [x] 1.1 Put the rule for a keyword's reference topic beside `referencePageOf`
...
- [x] 1.2 Have `referenceTopic` in `src/app/docsTopic.ts` call through to it, so
      the IDE's click-menu row keeps the behaviour it has today and the rule is
      stated once.
- [x] 1.3 Cover the new leaf in `src/dialects/referencePage.test.ts`: the topic
...
- [x] 2.1 End the reference-backed explanation in `src/lsp/hover.ts` with a link
      ...
      without one.
- [x] 2.2 Extend `src/lsp/hover.test.ts`: a keyword the reference covers offers
...
- [x] 3.1 Build completions in `src/lsp/completion.ts` from every autocomplete
      source the document's own editor state carries, rather than from the
      dialect's keyword source alone.
- [x] 3.2 Carry each source's own replaced range on the items it produced, in
      place of the single range shared by every item today.
- [x] 3.3 Extend `src/lsp/completion.test.ts`: names in scope are offered
...
- [x] 3.4 Note on the existing crunched-anchor test why it still holds now that
...
- [x] 4.1 Check `docs/contributing/architecture.md` against what moved: a new
...
- [x] 5.1 `npx vitest run src/lsp/ src/dialects/referencePage.test.ts src/app/`
      — the suites this change reaches. The full suite is CI's job.
- [x] 5.2 `npm run typecheck`
- [x] 5.3 `npm run lint`
- [x] 5.4 `npm run format:check` (or `npm run format` to fix, then re-check)
- [x] 5.5 Drive the real server the way an editor does, rather than only the unit
