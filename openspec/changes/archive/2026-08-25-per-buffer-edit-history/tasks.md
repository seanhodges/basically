...
- [x] 1.1 Add `src/editor/bufferHistory.ts`: snapshot a view's state with
...
- [x] 1.2 Make restore fall back to a fresh state built from the buffer's
      current text when a snapshot is missing or cannot be restored, so a bad
      snapshot costs history and never the buffer.
- [x] 1.3 Add `src/editor/bufferHistory.test.ts`: a round trip preserves
...
- [x] 2.1 Add `src/editor/editorCommands.ts` holding the view-generic Edit-menu
...
- [x] 2.2 Rework `CodeMirrorHost` to run the shared commands plus its own
      renumber cases, with no behaviour change while the BASIC tab is showing.
- [x] 2.3 Add `src/editor/editorCommands.test.ts` driving each command against a
      headless `EditorView`, including cut refusing to delete when the clipboard
      write fails.

## 3. Per-buffer state in the BASIC editor

- [x] 3.1 Extract the inline extension array in `CodeMirrorHost`'s mount effect
      into a builder function used by both the mount path and every restore.
- [x] 3.2 Give `CodeMirrorHost` the id of the buffer it is showing; on a change,
      snapshot the outgoing buffer and `view.setState` the incoming one.
- [x] 3.3 Have `Workspace` pass that buffer id from the active tab.
- [x] 3.4 Stop `withActiveTab` in `src/app/store.ts` pushing text through the
      document-override channel; it only sets the active tab now.
- [x] 3.5 Drop a buffer's snapshot wherever that buffer's content is replaced,
      ...
      across an Open.
- [x] 3.6 Extend `src/app/store.test.ts`: switching tabs no longer bumps the
...
- [x] 4.1 Stop keying `AsmEditor` by block id in `Workspace`, and swap block
...
- [x] 4.2 Subscribe `AsmEditor` to the edit-command channel, acting only while a
      block tab is showing, and make `CodeMirrorHost` stand down in that case.
- [x] 4.3 Add search to `AsmEditor`'s extensions and mirror the find-panel open
      state into the store the way `CodeMirrorHost` does; close the panel on a
      tab switch.
- [x] 4.4 Apply the external-bytes re-seed so it does not enter history.
- [x] 4.5 Drop a block's snapshot when the block is deleted.

## 5. Toolbar

- [x] 5.1 Disable Renumber, Renumber file and Procedures in both the desktop
      Edit menu and the mobile overflow menu while a block tab is showing.
- [x] 5.2 Add a colocated test that the BASIC-only entries are unavailable on a
      block tab and available on the BASIC and scratch tabs.

## 6. Quality gates

- [x] 6.1 Extend the existing undo/redo journey in
...
- [x] 6.2 Add one scenario under `e2e/memory-blocks/` — browser-only because it
      ...
      is still there.
- [x] 6.3 Run `npm run typecheck && npm test && npm run lint && npm run format:check`.
- [x] 6.4 Run `npm run e2e:chromium -- e2e/code-editor` and
