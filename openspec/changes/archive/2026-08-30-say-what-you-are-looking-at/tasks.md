...
- [x] 1.1 Five icons in the shared icon set (`src/components/icons.tsx`), under
...
- [x] 1.2 One mapping from a tab's kind to its icon in
      ...
      a name.
- [x] 1.3 The program's tab gains its icon, and with it the name span every
...
- [x] 1.4 The three items in the tab-creation menu each lead with the icon of
...
- [x] 1.5 The kind mark's own rule sets only opacity and font size today; an
      inline SVG needs the layout that keeps it on the text's baseline rather
      than sitting proud of it.
- [x] 1.6 Look at the five together, at the size the strip gives them, in one
...
- [x] 2.1 A small pure helper for how a block's extent reads: an address and a
...
- [x] 2.2 New shared bar component and its CSS module. It renders the block's
      ...
      names the block.
- [x] 2.3 Adopt it in `src/components/AsmEditor.tsx` and
...
- [x] 2.4 Move the Hex/Text choice into the bar's control slot and remove its
...
- [x] 2.5 The read-only mark: a short abbreviation at the very end of a saved
...
- [x] 2.6 Drop the read-only branch's flashed refusal in the byte editor's
...
- [x] 3.1 Add size to the block settings draft, its seeding from a block, its
...
- [x] 3.2 Apply it through the existing pure `setLength`
...
- [x] 3.3 Render it in `src/components/BlockSettingsDialog.tsx` in place of the
...
- [x] 3.4 Remove the length draft state, its commit handler and its input from
      `src/components/ByteEditor.tsx`, along with the "whole number of bytes"
      refusal, which is now the dialog's to report.
- [x] 3.5 Establish what a size change applied through the store does to the
      ...
      *Answered by driving it in a browser: the history was not lost but stale,
      and undoing spliced a stretch of the old projection into the new one. A
      reseed that rewrites the document now installs a fresh state, so the
      history goes with the document it described; a move, which leaves the
      document alone, keeps its history and still undoes the last byte edit.
      The size field says so beside the ceiling.*

## 4. Tests

- [x] 4.1 Unit tests for the extent helper: zero bytes (address alone), one
      byte (a range of one), a block ending at `$FFFF`, and the ordinary case.
- [x] 4.2 Extend `src/app/blockEdit.test.ts` for the size field: a valid size,
...
- [x] 4.3 No component tests — this repo tests pure modules plus Playwright,
...
- [x] 5.1 `e2e/memory-blocks/asm-editor.spec.ts` and
...
- [x] 5.2 Every use of the byte-count field's test id
...
- [x] 5.3 `e2e/persistence/saved-data-tabs.spec.ts`: the byte-count assertion
...
- [x] 5.4 Several specs match a tab strip's text with regular expressions
...
- [x] 5.5 `e2e/code-editor/scratch-buffers.spec.ts` reads an overflow item's
...
- [x] 5.6 No new e2e test for the marks themselves. They are `aria-hidden`, so
...
- [x] 5.7 Extend the existing journeys in those files; do not add a new cold
      page load for any of this.

## 6. Documentation

- [x] 6.1 `docs/guide/machine-code.md`: the bullet directing the reader to the
...
- [x] 6.2 `docs/guide/machine-code.md`: the **Settings** bullet in the tab-menu
      list now sets the block's size as well as its name, address, kind, entry
      and comment.
- [x] 6.3 `docs/guide/testing-programs.md`: "The bytes are shown read-only" now
...
- [x] 7.2 `npm run docs:build` (docs/ changed in group 5).
- [x] 7.3 `npm run e2e:chromium -- e2e/memory-blocks`,
...
- [x] 7.4 Phone-portrait viewport, through a throwaway Chromium spec rather
