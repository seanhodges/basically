...
- [x] 1.1 Add `src/dialects/binaryFormatLookup.ts`: given an optional file name,
      return every registered dialect whose `binaryImports` declares a matching
      extension.
- [x] 1.2 Add a colocated `src/dialects/binaryFormatLookup.test.ts` that iterates
...
- [x] 2.1 Add `src/ops/convert.ts`: `convertOp: Operation<ConvertInput,
...
- [x] 2.2 Confirm `importProgram` has no browser/DOM/store dependency that would
      violate `src/ops/`'s import restrictions (checked by `eslint.config.js`);
      relocate or re-export it if it does.
- [x] 2.3 Add a colocated `src/ops/convert.test.ts`, modeled on
...
- [x] 2.4 Register `convertOp` in `src/ops/registry.ts`'s `OPERATIONS` array
      (append, since the array's order is load-bearing for the assistant's
      tool-definition cache).

## 3. Caller parity

- [x] 3.1 Add the `convert`/`assistant` exemption to `src/ops/parity.ts`'s
...
- [x] 3.2 Add `convert` entries to `src/ops/parity.test.ts`'s `argv` and
      `inputs` maps (these tests iterate `OPERATIONS` generically and need a
      case for every operation).

## 4. Command-line wiring

- [x] 4.1 Add `'convert'` to the `OPERATIONS` list and a `ConvertArgs` type in
      ...
      `parseArgs`'s switch.
- [x] 4.2 Add the `OPERATION_USAGE.convert` help block and its `SUMMARY` line in
      `src/cli/usage.ts`, pointing to `basically build` for the reverse
      direction.
- [x] 4.3 Add a binary-safe file reader alongside `readProgram` in
      ...
      switch.
- [x] 4.4 Add `parseConvert` unit tests alongside the existing `parseBuild`
      coverage for `src/cli/args.ts`.

## 5. Spec sync and quality gates

- [x] 5.1 Note that `openspec/specs/headless-cli/spec.md` picks up this
...
- [x] 5.2 Run `npm run typecheck`, `npm run lint`, and `npm run format:check`
