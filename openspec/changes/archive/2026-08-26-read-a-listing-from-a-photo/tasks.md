...
- [x] 1.1 `src/ai/providers/types.ts` — `ChatImage.mediaType` becomes
...
- [x] 1.2 `src/app/screenCapture.ts` — narrow `ScreenCapture.mediaType` back to
...
- [x] 1.3 One colocated test per backend (`src/ai/providers/anthropic.test.ts`,
...
- [x] 2.1 New `src/app/listingPhoto.ts`, beside the app's other two DOM-image
...
- [x] 2.2 Scale bounded **twice** — by the long edge and by total pixels — and
...
- [x] 2.3 Decode with `imageOrientation: 'from-image'`, with the
...
- [x] 2.4 Draw with smoothing **enabled** — deliberately the opposite of
      ...
      other later.
- [x] 2.5 Encode as JPEG, with a quality ladder that re-encodes smaller if the
      ...
      request.
- [x] 2.6 The failure reasons, each one the user can act on: not a picture;
      ...
      unsupported file.
- [x] 2.7 Colocated `src/app/listingPhoto.test.ts`: the scale never upscales, is
...
- [x] 3.1 `src/storage/files.ts` — an image opener beside the binary one, using
      ...
      camera.
- [x] 3.2 Use `accept="image/*"` and **no `capture` attribute**, with a comment
...
- [x] 3.3 The one function all three routes reach: prepare the picture, reveal
...
- [x] 3.4 Do **not** demand a key on attach. The send path already demands it at
...
- [x] 3.5 The composer's paste route, reading the clipboard's **files**, not its
...
- [x] 3.6 `src/app/fileCommands.ts` — a picture branch ahead of the
...
- [x] 4.1 `src/ai/aiStore.ts` — a photograph is its own field on a request, not
      ...
      single line.
- [x] 4.2 The precedence: a photograph takes the slot ahead of any screen. The
...
- [x] 4.3 Fix the report that comes apart from its picture. A run composes its
...
- [x] 4.4 The composer's attachment lives in the store beside the pending fix, so
      clearing the conversation takes it too — a picture waiting to be sent is
      part of the conversation being cleared.
- [x] 4.5 The thread marks a turn as having shown a photograph, distinctly from
...
- [x] 4.6 Colocated tests in `src/ai/aiStore.test.ts`, where the real risk is and
...
- [x] 5.1 `src/ai/promptBuilder.ts` — the "a screen is attached" flag on the user
...
- [x] 5.2 Write the transcription guidance for the listing case, covering only
...
- [x] 5.3 The one bullet that deliberately overrides the general rule about
...
- [x] 5.4 Keep every word of it in the user turn. The system prompt is composed
...
- [x] 5.5 Add the negative assertion to `src/ai/promptStability.test.ts`: the
...
- [x] 5.6 Colocated tests in `src/ai/promptBuilder.test.ts`: the listing guidance
...
- [x] 6.1 `src/components/AiPanel.tsx` — an attach control beside Send, labelled
...
- [x] 6.2 The attached picture shown before it is sent: a thumbnail of the
...
- [x] 6.3 Say what is displaced, and only when something is: where a screen is
...
- [x] 6.4 Send accepts a photograph with no words, filling the request with a
...
- [x] 6.5 The turn's note reads as a photograph where it showed one, on the same
...
- [x] 6.6 `src/components/AiPanel.module.css` — the chip row above the composer.

## 7. Documentation

- [x] 7.1 `docs/guide/getting-started.md` — a short passage under the AI section:
...
- [x] 7.2 Do not touch the sidebar in `docs/.vitepress/config.ts`. A new passage
      on an existing page needs nothing there, and the sidebar changes only when
      the user asks.

## 8. The browser test

- [x] 8.1 A generated fixture at the flat `e2e/` root beside the existing stubs:
...
- [x] 8.2 Extend the existing journey in `e2e/ai-assistant/shown-screen.spec.ts`
...
- [x] 8.3 Comment what only a browser proves — a real file decoded, downscaled and
...
- [x] 8.4 Re-read, and reword, the existing assertion that the composer offers no
...
- [x] 8.5 Record the manual check the suite deliberately does not automate: take a
...
- [x] 9.5 `npm run docs:build` — docs change in group 7.
- [x] 9.6 `npm run e2e:chromium -- e2e/ai-assistant`. Only check off when it
      passes; a failing run leaves this unchecked with a note on what failed.
- [x] 9.7 `npm run e2e:chromium -- e2e/persistence` — the drop route changes a
