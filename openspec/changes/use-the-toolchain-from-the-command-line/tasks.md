...
- [ ] 1.1 Create `src/cli/args.ts` holding the whole grammar as a pure function
      ...
      exit code 1.
- [ ] 1.2 Support the five operations and their options: `machines [--json]`;
      ...
      second one.
- [ ] 1.3 Create `src/cli/usage.ts` with the help text: a summary listing every
      operation, and per-operation help. `--help`/`-h` with no operation prints the
      summary; with an operation, that operation's own help.
- [ ] 1.4 Write `src/cli/args.test.ts`: every operation parses; `-` and an absent
...
- [ ] 2.1 Create `src/cli/machines.ts` returning the registered machines as data:
...
- [ ] 2.2 Create `src/cli/info.ts` returning one machine's full description, every
...
- [ ] 2.3 Write `src/cli/info.test.ts` as a registry-driven table — one `it` looping
...
- [ ] 3.1 Create `src/cli/lint.ts` calling the dialect's `lint()` dry-run and
...
- [ ] 3.2 Write `src/cli/lint.test.ts`: a clean listing yields no problems and is
...
- [ ] 4.1 Create `src/cli/build.ts` returning the produced files as
...
- [ ] 4.2 Write `src/cli/build.test.ts`: selection by explicit id, selection by
...
- [ ] 5.1 Rewrite `scripts/headless/cli.mts` as a shim over `src/cli/`: read argv,
      ...
      error.
- [ ] 5.2 Apply the stream rule everywhere: standard output carries only the
      ...
      a build wrote.
- [ ] 5.3 Apply the exit-code rule: 0 succeeded; 1 the caller's mistake (the
      `RunError` path); 2 the program is at fault (a fatal problem while checking,
      building or running).
- [ ] 5.4 Reshape `run`'s flags: `--screenshot <path>` replaces `--png` and the
...
- [ ] 5.5 `git mv scripts/run-listing.sh scripts/basically.sh`, and update the
...
- [ ] 5.6 Confirm `scripts/headless/build.mjs` still needs no change — it bundles
...
- [ ] 6.1 Update the commands section of `CLAUDE.md`: replace the two
...
- [ ] 7.1 `npx vitest run src/cli/ src/dialects/headless/ src/dialects/screenPaints.test.ts`
...
- [ ] 7.3 `npx openspec validate --specs`
- [ ] 7.4 No e2e run: `headless-cli` has no browser surface, and nothing in the
...
- [ ] 7.5 By hand, with the bundle rebuilt from a clean tree, confirm each
...
- [ ] 7.6 By hand, confirm the ROM-less promise: with `machines`, `info`, `lint` and
