...
- [ ] 5.7 Disable the Describe-it option when no API key is set, with a plain note (not a link) that the AI assistant must be configured in settings before the option becomes available; read key presence once when the dialog opens, since it cannot change while the dialog is up
- [ ] 5.8 Pre-select the active machine and Blank on open, and support Escape to cancel, Enter to create, and autofocus on the name field
- [ ] 5.9 Mount `<NewProjectDialog />` in `src/App.tsx` alongside the other dialogs
- [ ] 5.10 Add a test covering the gate: with no key the Describe-it option is disabled and carries the note, and the other starting points still work; with a key set it is selectable
