...
- [x] 1.1 Add `src/lsp/headlessEditorState.test.ts`: for a crunched machine, an
...
- [x] 1.2 In the same file, assert a reference page loads through `import()` and
      yields a row with `syntax` and `description` for a known keyword.
- [x] 1.3 Add `vscode-languageserver` and `vscode-languageserver-textdocument`
      to `dependencies` in `package.json`. Both MIT; note the licence check in
      the commit message, per `openspec/config.yaml`.

## 2. The operation

- [x] 2.1 `src/cli/args.ts`: add `'lsp'` to `OPERATIONS`, an `LspArgs` member to
...
- [x] 2.2 `src/cli/usage.ts`: add `OPERATION_USAGE.lsp` (`args.test.ts` loops
...
- [x] 2.3 Extend `src/cli/args.test.ts`: `--stdio` parses, an unknown option is
...
- [x] 3.1 `src/lsp/binding.ts`: resolve a configured machine through the existing
      `findMachine` from `src/dialects/headless/runListing.ts`, so the server and
      `-m` accept the same spellings.
- [x] 3.2 Same file: `inferMachine(source)` built on `computeCompatibleDialects`
...
- [x] 3.3 Same file: the precedence chain as one pure function over (declared,
      ...
      on it.
- [x] 3.4 `src/lsp/binding.test.ts`: registry-driven. Every registered dialect's
...
- [x] 4.1 `src/lsp/documents.ts`: an open-document store keyed by URI, holding
      text, version and bound dialect, and caching an `EditorState` per
      `(version, dialect id)` so a keystroke does not rebuild it twice.
- [x] 4.2 Same file: `offsetToPosition` / `positionToOffset` over the document
...
- [x] 4.3 `src/lsp/documents.test.ts`: round-trip offsets and positions across
...
- [x] 5.1 `src/lsp/diagnostics.ts`: `dialect.lint(source)` plus
...
- [x] 5.2 Same file: the declined-binding diagnostic, at the first line, naming
      the setting to configure.
- [x] 5.3 `src/lsp/diagnostics.test.ts`: `10 PRINT "HI` on `zx81` is fatal at
...
- [x] 6.1 `src/lsp/completion.ts`: drive `dialect.completionSource` from a
...
- [x] 6.2 Same file: construct templates pass through as snippet insert-text
      unchanged — `src/editor/constructs.ts` already writes `${1:I}` … `${0}`,
      which is the protocol's own syntax.
- [x] 6.3 Same file: carry the replacement range the source computed, so the
      crunch re-anchoring in `src/editor/completions.ts` survives translation.
- [x] 6.4 `src/lsp/completion.test.ts`: a keyword prefix offers that machine's
...
- [x] 7.1 `src/lsp/hover.ts`: `referenceTokenAt(state, pos, BASIC_REFERENCE_KINDS,
      operatorSpellings(dialect))` then `lookupWord(text,
      keywordSpellingsFor(dialect.id))` to resolve a short spelling to its
      keyword.
- [x] 7.2 Same file: load the reference page through the existing per-page
      ...
      to protect.
- [x] 7.3 Same file: compose `syntax` + `description` + `tag` into markup, and
      fall back to the dialect's own `signature`/`doc` where the page has no row.
- [x] 7.4 `src/lsp/hover.test.ts`: hovering `PRINT` on `commodore64` yields its
...
- [x] 8.1 `src/lsp/definition.ts`: a line reference is an integer literal after
...
- [x] 8.2 `src/lsp/symbols.ts`: `buildOutline(source,
      outlineCapabilities(dialect.keywords))` to document symbols, with ranges
      from `ProcRegion` where there is one.
- [x] 8.3 `src/lsp/references.ts`: `findVariableUsages` to locations and document
      highlights, converting its document offsets through the document store.
- [x] 8.4 `src/lsp/definition.test.ts`, `symbols.test.ts`, `references.test.ts`:
...
- [x] 9.1 `src/lsp/handlers.ts`: one entry point per protocol method, taking the
...
- [x] 9.2 `scripts/headless/lsp.mts`: `createConnection`, `TextDocuments`
...
- [x] 9.3 Same file: divert `console.log`/`info`/`debug` to standard error for
...
- [x] 9.4 `scripts/headless/cli.mts`: a `case 'lsp'` in `main()`'s switch. It is
      the one operation whose normal path does not return an exit code.
- [x] 9.5 Debounce diagnostics at 400 ms, matching the editor's linter.

## 10. Documentation

- [x] 10.1 `docs/contributing/architecture.md`: add `src/lsp/` to the layers
...
- [x] 10.2 `CLAUDE.md`: the commands block gains the operation, with a one-line
      example of an editor launching it.
- [x] 10.3 Write the user-facing page under `docs/guide/` explaining how to wire
      ...
      links.
- [x] 10.4 Add that page to the `sidebar` in `docs/.vitepress/config.ts`. The
...
- [x] 11.1 `npx vitest run src/lsp/ src/cli/ src/editor/ src/dialects/registry.test.ts`
      — the new suites, the arg grammar they extend, and the editor modules that
      now have a second caller.
- [x] 11.2 `npm run typecheck && npm run lint && npm run format:check`.
- [x] 11.3 `npm run docs:build`, because `docs/contributing/architecture.md` and
      a `docs/guide/` page change.
- [x] 11.4 `npx openspec validate --specs`.
- [x] 11.5 No e2e run. `language-server` has no browser surface, and no existing
      ...
      with no `e2e/` folder.
- [x] 11.6 By hand, with the bundle rebuilt:
