...
- [ ] 1.1 `src/storage/settings.ts`: the browser tab's own id — generated on
...
- [ ] 1.2 `src/storage/vfs/db.ts`: `VfsFileDoc` gains `tabId`, and the
...
- [ ] 1.3 The schema change that costs: version 1, a migration strategy that
...
- [ ] 1.4 A row the store cannot read back — no tab id, bytes that will not
      decode, a missing field — is deleted where the restore meets it, not
      skipped, so it cannot be met again on the next restore.
- [ ] 1.5 The reclaim of rows whose tab is gone: a last-seen registry of tab ids
...
- [ ] 2.1 `src/storage/vfs/vfsStore.ts`: `setDialect(id)` as the one writer of
      ...
      before it re-tags.
- [ ] 2.2 `hydrate(dialectId): Promise<void>`: merge the rows for this tab and
...
- [ ] 2.3 Mounted saves are never persisted: a `save` marked `mounted` mirrors as
...
- [ ] 2.4 `clearMounted()`: drop the mounted entries from memory only — no
      purge, no generation bump — for the run effect to call before the machine
      re-mounts what it needs.
- [ ] 2.5 Memory-only mode (`setPersistence(false)` or equivalent): the mirror,
      the purge and `hydrate` all become no-ops. For the player.
- [ ] 2.6 Rewrite the module's header comment: the lifetime rules it states are
...
- [ ] 3.1 `src/storage/vfs/vfsStore.test.ts`: hydrate merges rows into memory;
...
- [ ] 3.2 Tab isolation, against one shared in-memory storage: two stores with
      ...
      two saves outright.
- [ ] 3.3 The reclaim: rows of a tab id the registry no longer vouches for are
      purged at boot, and rows of a tab it does vouch for survive.
- [ ] 3.4 A row that cannot be read back is deleted rather than skipped: the
      restore that meets it leaves the collection without it, and the rest of
      the tab's files still land.
- [ ] 3.5 The two ordering traps, which are the ones worth having: a `clear()`
...
- [ ] 3.6 A mounted save writes no row and removes any existing row under that
      name; `clearMounted()` drops mounted entries and leaves the rest.
- [ ] 3.7 Hydrate resolves rather than rejects when the collection cannot be
      opened, and memory-only mode neither reads nor writes the database.

## 4. The emulator lifecycle stops clearing

- [ ] 4.1 `src/components/EmulatorPane.tsx`: remove all four `emulatorVfs.clear()`
      calls (run, reset, unmount, machine/ROM change).
- [ ] 4.2 In the run effect, `await emulatorVfs.hydrate(dialect.id)` — placed
...
- [ ] 4.3 Check the comments this invalidates: the run effect's "a start empties
...
- [ ] 5.1 A hook in `src/app/` that tags the store with the current machine and
...
- [ ] 5.2 `src/main.tsx`: put the store in memory-only mode on the player route,
      before the render, so a share link neither keeps its own files nor
      disturbs the IDE's.
- [ ] 5.3 Confirm the surviving discard points in `src/app/store.ts` still tag
...
- [ ] 6.1 Store: `pendingDeleteDataFile`, `requestDeleteDataFile(name)` (which
...
- [ ] 7.1 `src/app/store.test.ts`: the surviving discard points still discard
...
- [ ] 7.2 `src/app/dataBlocks.test.ts`: restored files project into tabs, and
      restored mounted entries never do.

## 8. Docs

- [ ] 8.1 `docs/guide/testing-programs.md`, "Reading the data your program
...
- [ ] 9.2 `npm run e2e:chromium -- e2e/persistence e2e/code-editor`. Rework the
