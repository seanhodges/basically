...
- [ ] 1.1 Add `src/dialects/machineDirective.ts` as a leaf module in the shape of
...
- [ ] 1.2 Same file: `readMachineDirective(source)` returning the declared name,
...
- [ ] 1.3 `src/dialects/machineDirective.test.ts`: recognition with and without
...
- [ ] 2.1 Read `openspec/changes/refer-to-blocks-by-name/design.md` first. That
...
- [ ] 2.2 Add the helper every caller uses: takes a source and a dialect (or no
...
- [ ] 2.3 Route every path that turns text into bytes through it. Working from
...
- [ ] 2.4 A test that fails if a path is missed: for every registered machine and
...
- [ ] 2.5 A test that a problem on a line after a declaration is reported at the
      line the user sees it on, on every registered machine.

## 3. The command line

- [ ] 3.1 `src/cli/args.ts`: `-m`/`--machine` stops being required for the
      ...
      declare nothing.
- [ ] 3.2 `src/cli/lint.ts` and `src/cli/build.ts`: take the machine from the
...
- [ ] 3.3 `src/cli/usage.ts`: say that a program declaring its machine need not be
      given one.
- [ ] 3.4 `src/cli/lint.test.ts`, `build.test.ts`, `args.test.ts`: a declaring
...
- [ ] 4.1 Opening a document that declares a machine makes it the active target.
- [ ] 4.2 Switching the active target rewrites an existing declaration, and adds
      none where the document has none. This rides on the existing switch
      conversation rather than adding one.
- [ ] 4.3 Colocated tests for both, including that a document with no declaration
      still has none after a switch.

## 5. Documentation

- [ ] 5.1 `docs/reference/file-formats.md`: the declaration alongside `#BIN`,
      described as an end-user page — no source paths, no internal symbols.
- [ ] 5.2 `docs/contributing/architecture.md`: the above-the-seam pass, in the
...
- [ ] 5.3 `CLAUDE.md`: the commands block, where `-m` is shown, notes it is
      optional for a declaring program.

## 6. Quality gates

- [ ] 6.1 `npx vitest run src/dialects/ src/cli/ src/editor/ src/app/ src/share/`
...
- [ ] 6.3 `npm run docs:build`, because `docs/reference/file-formats.md` and
      `docs/contributing/architecture.md` change.
- [ ] 6.4 `npx openspec validate --specs`.
- [ ] 6.5 `npm run e2e:chromium -- e2e/code-editor e2e/project-setup` — this
...
- [ ] 6.6 By hand, with the bundle rebuilt:
