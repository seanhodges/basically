...
- [ ] 1.1 Check whether the generated auto-loader source
...
- [ ] 1.2 If the bytes differ, give `loaderTapBlocks` a tokenizer parameter
...
- [ ] 2.1 In `src/dialects/zxspectrum128/targets.ts`, give the tape layout the
...
- [ ] 2.2 Have both the `.TAP` and cassette `.wav` builders consume that one
      ...
      arguments through.
- [ ] 2.3 Set `supportsBlocks: true` on the 128's `tap-file` and `wav` targets,
      and pass `blocks`/`loader` from their `build` options.
- [ ] 2.4 Update the module's header comment: it currently explains only why the
...
- [ ] 3.1 Add a `zxspectrum128` case to
...
- [ ] 3.2 Assert the 128's loader-on tape is the **same shape** as the 48K's for
      ...
      layouts drifting.
- [ ] 3.3 Fix that file's header comment, which still says "ZX Spectrum today;
...
- [ ] 3.4 Check whether `e2e/memory-blocks/` needs anything. It should not: the
...
- [ ] 4.1 `docs/reference/zxspectrum/formats.md` — the 128 currently appears only
...
- [ ] 4.2 `docs/reference/file-formats.md` — the "Machine code & data blocks"
...
- [ ] 4.3 Check `docs/guide/machine-code.md` needs no change — it speaks of
