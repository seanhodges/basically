...
- [x] 1.1 New module `src/app/screenshot.ts` holding the whole path: the
...
- [x] 1.2 Enlargement: `max(1, round(target / width))` against a target width of
...
- [x] 1.3 Pixels come from `captureScreen()` in `src/app/screenCapture.ts` — do
...
- [x] 1.4 Filename: the program name as the existing exports derive it
...
- [x] 1.5 The entry point **resolves to a result** — saved, or not-saved with a
...
- [x] 1.6 SPDX header (`// SPDX-License-Identifier: GPL-3.0-or-later`) on every
      new file, as the rest of the tree carries.

## 2. Tests for the module

- [x] 2.1 New colocated `src/app/screenshot.test.ts`.
- [x] 2.2 Enlargement, driven off `src/dialects/registry.ts`: for **every
      ...
      machine count.
- [x] 2.3 Filename: an untitled document, a named one, a name needing
      truncation, and zero-padding of the timestamp fields.

## 3. The icon and the two surfaces

- [x] 3.1 A camera icon in `src/components/icons.tsx`, matching the size and
      stroke of its siblings.
- [x] 3.2 `src/components/Toolbar.tsx`: an `icon-btn` in the existing cluster
...
- [x] 3.3 The same action in the `⋯` mobile overflow menu — icon-only buttons
      are hidden on mobile, so a toolbar button alone leaves phones without it.
- [x] 3.4 `src/player/PlayerApp.tsx`: a button in the top bar's actions beside
...
- [x] 4.1 `src/app/shortcuts.ts`: a `run.screenshot` entry in the `Run`
      ...
      duplicate chord.
- [x] 4.2 Dispatch it in `src/app/useGlobalShortcuts.ts`, and show the hint on
      the toolbar button the way the neighbouring buttons do.

## 5. Browser proof

- [x] 5.1 Extend the existing journey in
...
- [x] 5.2 In the existing `e2e/sharing-player/player.spec.ts` journey, one
      assertion that the player's button is there. It is the same handler, so it
      does not need its own download round trip.
- [x] 5.3 Do **not** add a per-machine e2e matrix. The only per-machine fact is
      the enlargement factor, and that is task 2.2.

## 6. Documentation

- [x] 6.1 A row in the "Running and debugging" table of
      `docs/guide/keyboard-shortcuts.md` (hand-written, not generated).
- [x] 6.2 A short note under "Running a program" in
...
- [x] 7.5 `npm run docs:build` — this change edits `docs/`
- [x] 7.6 `npm run e2e:chromium -- e2e/program-execution`
- [x] 7.7 `npm run e2e:chromium -- e2e/sharing-player`
- [x] 7.8 Check off 7.6 and 7.7 only when those runs pass; a failing run leaves
