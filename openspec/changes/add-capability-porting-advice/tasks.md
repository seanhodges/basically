...
- [ ] 1.1 Create `docs/reference/data/domain-guidance.ts` with the
      `DomainGuidance` and example interfaces: `to`, `domain`, `support`,
      `summary`, `instead?`, `example?`, `reachFor?`
- [ ] 1.2 Document in the file header that cells are keyed by (target, capability)
...
- [ ] 1.3 Export the empty `domainGuidance` array so the crosscheck can be
...
- [ ] 2.1 Create `docs/reference/data/domain-guidance-crosscheck.test.ts`
      asserting structural validity: every `to` is a real page slug, every
      `domain` is in the vocabulary, no duplicate `(to, domain)`
- [ ] 2.2 Loss completeness — for each target, compute the union of `mustReplace`
      ...
      `instead` for each
- [ ] 2.3 Gain completeness — every domain the target has at least one command in
      requires a cell with a non-empty `summary`
- [ ] 2.4 No dead cells — fail on a cell whose domain is neither present on the
      target nor losable into it, and on an `instead` where nothing is losable
- [ ] 2.5 Example rule — `example` present exactly when `support !== 'full'` and
      `instead` is present
- [ ] 2.6 `reachFor` pinning — each name is a real row on that target's page and
      carries that domain
- [ ] 2.7 `support` honesty — `'none'` requires zero target commands in that
      domain, `'full'` requires at least one
- [ ] 2.8 Budgets — `MAX_SUMMARY_CHARS = 160`, `MAX_INSTEAD_CHARS = 200`,
      `MAX_EXAMPLE_LINES = 5`, `MAX_EXAMPLE_LINE_CHARS = 40`,
      `MAX_CAPTION_CHARS = 60`, `MAX_REACH_FOR = 4`
- [ ] 2.9 Add the bridge assertion to `porting-crosscheck.test.ts`: every
...
- [ ] 3.1 Pass one, `summary` + `support` for every mandatory cell (tests 2.3,
      2.7 green)
- [ ] 3.2 Pass one, `instead` for every mandatory cell (tests 2.2, 2.4 green)
- [ ] 3.3 Pass two, worked examples for `support: 'none'` cells
- [ ] 3.4 Pass two, worked examples for `support: 'partial'` cells (test 2.5
      green)
- [ ] 3.5 Add `reachFor` names where they help, and delete any cell the no-dead-
      cells check rejects

## 4. Diff and guidance logic

- [ ] 4.1 Add an optional `domainGuidance` input and a `domains` output map to
      `composeGuidance` in `docs/.vitepress/theme/dialectCompare.ts`, keeping it
      pure — the table is passed in, not imported
- [ ] 4.2 Extend `domainSections` to order groups `none` → `partial` → `full`,
      falling back to canonical vocabulary order, replacing the placeholder
      ordering from the preceding change
- [ ] 4.3 Add `CapabilityBrief` and `capabilityBrief(newlyAvailable, to, guidance,
...
- [ ] 4.4 Confirm `diffKeywords`, `falseFriendsBetween`, `diffEscapes`,
      `renameMap` and `operatorNames` are unchanged, and `dialectCompare.ts`
      still imports only types from the data layer
- [ ] 4.5 Add `dialectCompare.test.ts` cases for `composeGuidance`: the `domains`
      map is target-scoped, and empty when `domainGuidance` is omitted
- [ ] 4.6 Add `domainSections` ordering tests: `none` above `partial` above
      `full`, ties by vocabulary order, a group with no cell still renders its
      commands
- [ ] 4.7 Add `capabilityBrief` tests: per-domain counts, `reachFor` preferred
      over first-N, a `reachFor` name the source already has is dropped, ordered
      by gain, empty input yields `[]`
- [ ] 4.8 Confirm every pre-existing test in `dialectCompare.test.ts` still
      passes unmodified

## 5. Rendering

- [ ] 5.1 Render each lost-capability group's `instead` once for the group, above
      or below the comma run of command names — never repeated per command
- [ ] 5.2 Render the group's worked example as a captioned code block where one
      exists
- [ ] 5.3 Confirm `summary` is not rendered in lost groups and `instead` is not
      rendered in the brief, so neither side reads twice
- [ ] 5.4 Replace the "Newly available" list with the capability brief: one line
      per capability with its gain count, `summary`, a few named commands and a
      link into the target's reference page
- [ ] 5.5 Delete the `newlyAvailableList` truncation state; leave the other lists
      on `useTruncatedList` unchanged
- [ ] 5.6 Update the summary line so the additions are reported as capability
...
- [ ] 6.6 `npm run e2e:chromium -- e2e/porting-guidance`
- [ ] 6.7 `npx openspec validate --changes`
- [ ] 6.8 Manual check via `npm run docs:dev` at `/docs/reference/compare`:
...
- [ ] 6.9 Confirm the whole page for a distant pair is still readable in a few
