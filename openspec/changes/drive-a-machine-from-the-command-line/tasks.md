...
- [x] 1.1 Move `parseDriveScript`, `runDriveScript`, `describe`, `DriveAction`,
...
- [x] 1.2 Grow the parser: `#` comment lines are skipped; `PRESS` takes a `+`-joined
      ...
      proof.
- [x] 1.3 Give `MachineControl` (`src/app/machineControl.ts`) the two members
...
- [x] 1.4 Extend `runDriveScript` so `WAIT END` calls `waitForEnd`, and extend
      `describe` so it reads as a sentence in the report.
- [x] 1.5 Write `src/app/driveScript.test.ts` cases for the new lines: comments
      vanish; a chord presses its names together; the wait-for cap is read; `WAIT
      END` parses and runs.
- [x] 1.6 Add `programState` and `waitForEnd` cases to
...
- [x] 2.1 Create `src/keyboard/keyNames.ts` with `resolveKeyName(layout, name)`
...
- [x] 2.2 Make `pressKeys` in `src/app/machineControl.ts` resolve each name through
...
- [x] 2.3 Write `src/keyboard/keyNames.test.ts` - pure, no ROMs, fast, and
      ...
      rather than by hand.
- [x] 2.4 Add a case to `src/app/machineControl.test.ts` driving the ZX81 by
      vocabulary names (`A`, `ENTER`, `SPACE`) rather than ids, so the resolution is
      proved on a ROM and not only on data.
- [x] 2.5 Add `keys: string[]` to the description in `src/cli/info.ts` from
...
- [x] 2.6 No `src/dialects/keyNames.test.ts`. `src/ai/machineObservability.test.ts`
...
- [x] 3.1 Delete `driveKeyNames` from `src/ai/machineObservability.ts` and have
...
- [x] 3.2 Reword the one bullet of `buildDriveRules` that lists the machine's keys
...
- [x] 3.3 Repoint the every-machine ROM crosscheck in
...
- [x] 3.4 Add the wrong-key proof to `src/dialects/caseKeys.test.ts`, which already
...
- [x] 3.5 Run `src/ai/promptStability.test.ts` and re-record any budget that moved.
...
- [x] 4.1 Add `drive?: (machine: MachineEmulator, step: () => void) => void` to
...
- [x] 4.2 Write `src/dialects/headless/runListing.test.ts`: an `INKEY$` program on a
...
- [x] 5.1 In `src/cli/args.ts`, add `--keys <script>` to `run`, refusing `--keys`
...
- [x] 5.2 Create `src/cli/drive.ts`: `parseSchedule(text)` splits inline text on
...
- [x] 5.3 In `scripts/headless/cli.mts`, wire `run --keys`: refuse a ROM-less
...
- [x] 6.1 Update the commands section of `CLAUDE.md`: one `run --keys` example
      that gets past a prompt, and mention that it needs a ROM.
- [x] 6.2 Update `docs/contributing/architecture.md`: say that `run --keys` and the
...
- [x] 7.1 `npx vitest run src/app/ src/keyboard/ src/cli/ src/dialects/headless/ src/ai/`
...
- [x] 7.3 `npm run docs:build`, because `docs/contributing/architecture.md` changes.
- [x] 7.4 `npx openspec validate --specs`
- [x] 7.5 `npm run e2e:chromium -- e2e/ai-assistant` - this change now reaches a
      ...
      surface.
- [x] 7.6 By hand, with the bundle rebuilt: `./scripts/basically run <an INKEY$
