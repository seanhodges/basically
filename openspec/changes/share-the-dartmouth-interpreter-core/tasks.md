...
- [x] 2.1 Add `src/emulator/dartmouth/profile.ts` with a `DartmouthProfile`
...
- [x] 2.2 Add `src/dialects/ge235/profile.ts` holding the GE-235's values, each
      ...
      cross-reference.
- [x] 2.3 Take the profile in the `Interpreter` constructor and thread it to the
...
- [x] 2.4 Construct `Ge235InterpreterMachine` with the GE-235 profile, leaving
...
- [x] 2.5 Confirm `src/emulator/dartmouth/` has no import edge into
      `src/dialects/<name>/` — only into `src/dialects/types.ts`.
      `grep -rn "dialects/ge235" src/emulator/dartmouth/` must be empty.
- [x] 2.6 Decide where `FRAME_HZ` belongs (design.md leaves this open) and, if
      it stays in the core, say why in a comment so it does not read as an
      oversight.
- [x] 2.7 `npx vitest run src/dialects/ge235/ src/emulator/dartmouth/` — green,
...
- [x] 3.1 In `src/dialects/ge235/index.ts`: set
...
- [x] 3.2 Mirror the version and blurb in the `ge235` entry of
      `src/reference/machines.ts` (it already declares
      `basicFamily: 'Dartmouth BASIC'`).
- [x] 3.3 Update `basicDialect` in the `ge235` entry of `src/reference/facts.ts`.
- [x] 3.4 `npx vitest run src/dialects/registry.test.ts src/reference/` — the
