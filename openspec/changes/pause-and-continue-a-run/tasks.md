...
- [ ] 1.1 Add `src/app/runControl.ts`: `runControlStateOf(status)` returning
...
- [ ] 1.2 Add `src/app/runControl.test.ts`: the state mapping with an
...
- [ ] 2.1 Add `pauseRequest` and `requestPause()` to `src/app/store.ts`,
      ...
      a second counter.
- [ ] 2.2 Extend `src/app/store.test.ts`: `requestPause()` bumps `pauseRequest`
      and leaves the rest of the run state alone.

## 3. Run loop

- [ ] 3.1 Add the pause effect to `src/components/EmulatorPane.tsx`, between the
...
- [ ] 3.2 Guard the pause effect with `canPauseRun` — refuse while an assistant
      run check is armed, while the assistant is driving the machine, and before
      the first frame has been drawn.
- [ ] 3.3 Replace the continue effect's debug-session guard with "a machine
...
- [ ] 3.4 Extend `src/app/runTiming.test.ts`: pausing with no line sets the
      `paused` ending and resuming restores `running` without losing elapsed
      time (only line numbers have been passed to it before).

## 4. The run control

- [ ] 4.1 Drive the run button over the editor in `src/components/Workspace.tsx`
      ...
      `data-state={state}`.
- [ ] 4.2 Style the paused and running states in
...
- [ ] 7.2 Extend the existing orientation-change test in
      ...
      name — the toolbar's Play and Continue are present at that viewport and
      accessible-name matching is a substring match. Leave the run paused so the
      test's existing flip-back assertions still hold.
...
- [ ] 8.6 `npm run e2e:chromium -- e2e/program-execution e2e/ai-assistant`
