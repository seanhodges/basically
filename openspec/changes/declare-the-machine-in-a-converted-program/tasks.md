...
- [x] 1.1 Add the writing half to `src/dialects/machineDirective.ts`: a function
      returning the `#MACHINE <id>` line for a machine id, beside the existing
      recogniser and parser, importing nothing new.
- [x] 1.2 Extend `src/dialects/machineDirective.test.ts` so what the new function
...
- [x] 2.1 Add the optional `declareMachine` boolean to `ConvertInput` and to
      `convertOp`'s input schema in `src/ops/convert.ts`, documented in the same
      voice as its neighbours.
- [x] 2.2 Prepend the declaration to the returned source when it is asked for,
      naming the dialect the operation resolved, using the function from group 1.
- [x] 2.3 Leave `describe` reporting what it reports today; the declaration is
      part of the source, not a separate finding.

## 3. Give the command line the same option

- [x] 3.1 Add the flag to `parseConvert` in `src/cli/args.ts`, mapped to the
      operation's input property.
- [x] 3.2 Add it to the `convert` entry in `src/cli/usage.ts`, in the shape the
      other options there use.
- [x] 3.3 Extend `src/cli/args.test.ts`: the flag parses, its absence leaves the
      property unset, and the parsed input still validates against the schema.

## 4. Pin the behaviour

- [x] 4.1 Extend `src/ops/convert.test.ts`: the declaration is present when asked
...
- [x] 4.2 Cover the pipeline fact the design names — a listing whose text
      declares a machine, built and then converted, comes back with no directive
      of its own, so the prepend never doubles.
- [x] 4.3 Confirm `src/ops/parity.test.ts` still passes unchanged: this adds an
      input property, not a route, so no caller gains or loses an exemption.

## 5. Quality gates

- [x] 5.1 `npx vitest run src/dialects/machineDirective.test.ts src/ops/convert.test.ts src/ops/parity.test.ts src/cli/args.test.ts`
- [x] 5.2 `npm run typecheck`
- [x] 5.3 `npm run lint`
- [x] 5.4 `npm run format:check` (or `npm run format` and re-run)
- [x] 5.5 By hand, against a real file: `./scripts/basically convert prog.p` and
