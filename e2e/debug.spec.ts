 ...
 *
 *  1. Core flow — arm Debug, set a breakpoint in the gutter, Run pauses on it,
 *     Step advances to the next BASIC line, Continue re-pauses on the
 *     breakpoint, Stop clears the session.
 ...
 *     works afterwards.
 *  3. Capability gating — the Debug toggle only shows for debuggable dialects
 *     (hidden for the Commodore 64).
 *
...
/** The debugger's Step/Continue/Stop bar lives next to the Step button. */
function debugBar(page: Page) {
  return page.getByRole('button', { name: 'Step' }).locator('..');
}
...
test('core flow: breakpoint, run-to-pause, step, continue, stop', async ({
  ...
  await expect(debugToggle).toHaveClass(/active/);

  // Set a breakpoint on line 20 — a red dot appears in the gutter.
  await toggleBreakpointOnLine(page, 20);
  await expect(page.locator('.cm-breakpoint-gutter')).toContainText('●');

  // Run pauses just as line 20 starts executing.
  ...
  await debugBar(page).getByRole('button', { name: 'Step' }).click();
  ...
  await debugBar(page).getByRole('button', { name: 'Continue' }).click();
  await expect(page.getByText('paused at line 20')).toBeVisible({
    timeout: 20_000,
  });

  // Stop ends the session and clears the highlight.
  await debugBar(page).getByRole('button', { name: 'Stop' }).click();
...
test('Debug toggle is gated to debuggable dialects', async ({ page }) => {
  await open(page);
  // Default target is the ZX81 — debuggable.
  await expect(page.getByRole('button', { name: 'Debug' })).toBeVisible();

  // The Commodore 64 wraps a core that can't single-step: no Debug toggle.
  await page
    .locator('select.dialect-select')
    .selectOption({ label: 'Commodore 64' });
  await expect(page.getByRole('button', { name: 'Debug' })).toHaveCount(0);

  // Switching back to a Sinclair machine brings it back.
  await page
    .locator('select.dialect-select')
